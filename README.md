...
a declared boundary, explicit contracts, and observable behaviour. A cell may vary
in size — a single function, a service, a team's entire domain — but it must
remain governable, replaceable, and externally accountable as a single unit. What
makes it a cell is not its size or technology but its properties: it declares what
it does, exposes how it behaves, and can be held accountable through externally
verifiable evidence without access to its internal reasoning. A cell operated by
an AI system and a cell operated by a team of engineers are governed identically.
That is the point.
